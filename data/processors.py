"""
    data / processors.py
    --------------------
    Data-processor retrieves the loaded data from pd.DataFrame 
    and checks the structure of data and validate it based on 
    the expected appearance of datasets
"""
...
class DataProcessor:
    ...
    def process_chunk(self, chunk: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        for column, spec in self.SCHEMA.items():
            ...
            if values.size == 0:
                p[column] = np.empty((0,), dtype=dtype)
                continue
            
            try:
                if not stacked:
                    p[column] = self._convert_simple_column(values, dtype, column)
                else:
                    p[column] = self._convert_stacked_column(values, dtype, column)
            
            except Exception as e:
                print(f"Warning: Failed to process column: '{column}', dtype=object")
                p[column] = np.asarray(values, dtype=object)
    ...
    def concatenate(self,results: List[Dict[str, np.ndarray]]) -> Dict[str, np.ndarray]:
        ...
        for key in first_result.keys():
            ...
            for result in results:
                if key in result: arrays.append(result[key])
